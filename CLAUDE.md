...
- `src/main.cpp` — `app_main()` wires modules together: bring up the relay GPIO, bring up Wi-Fi, bring up MQTT, publish a `heartbeat` presence message and the initial relay state, then loop polling for MQTT commands every `COMMAND_POLL_INTERVAL_MS` (100ms) and applying/echoing any new relay state. The same loop re-publishes the heartbeat every `HEARTBEAT_INTERVAL_MINUTES` (default 60) by accumulating elapsed poll ticks in `ms_since_heartbeat` — there's no separate timer/task. Modules don't call each other directly — `main.cpp` is the only place that reads one module's output and feeds it into another.
...
- `src/mqtt_client_manager.{h,cpp}`:
  - `mqtt_client_manager_setup()` connects over TLS (`mqtts://`) with mutual authentication — `broker.verification.certificate` (root CA, validates the broker) plus `credentials.authentication.certificate`/`.key` (this device's client cert/key, presented to the broker) alongside the existing username/password. Connection is event-group-gated on `MQTT_EVENT_CONNECTED`, and on connect it subscribes to `MQTT_TOPIC_RELAY_COMMAND`. TLS and certificate verification are both optional, gated by `#if MQTT_BROKER_USE_TLS` / `#if MQTT_BROKER_VERIFY_CERTIFICATE` (see below) — with `MQTT_BROKER_USE_TLS 0` the broker URI switches to plain `mqtt://` and no certs are configured at all; with TLS on but `MQTT_BROKER_VERIFY_CERTIFICATE 0`, `broker.verification.certificate` is left unset, which makes esp-tls fall back to `MBEDTLS_SSL_VERIFY_NONE` (connection still encrypted, server cert not checked) — client mutual-auth cert/key are still presented either way.
  - `mqtt_client_manager_publish_heartbeat()` builds the heartbeat-message JSON by hand (fixed fields, no cJSON needed) and publishes it.
...
`mqtt_broker_config.h` currently defines: `MQTT_BROKER_HOST`, `MQTT_BROKER_PORT` (8883, TLS), `MQTT_BROKER_USERNAME`/`PASSWORD`, `MQTT_TOPIC_HEARTBEAT`, `MQTT_TOPIC_RELAY_COMMAND`, `MQTT_TOPIC_RELAY_STATE`, `MQTT_BROKER_USE_TLS` / `MQTT_BROKER_VERIFY_CERTIFICATE` (`1`/`0` toggles, see above), and the TLS material `MQTT_BROKER_ROOT_CA`, `MQTT_BROKER_CERTIFICATE`, `MQTT_BROKER_PRIVATE_KEY` (PEM strings, `\n`-per-line, no trailing `;` inside the `#define` — a stray `;` or `=` inside these multi-line macros silently breaks the field assignment or fails to compile).
